...
- ローカルファーストで設計する。
- 情報ソース、マップデータ、抽出結果、マップ、施策、バージョンはアプリ側で管理する。
...
| Local DB | SQLite | 決定 | ローカルファーストのマップ管理に向いている |
...
| AI integration | Codex App Server + Cursor SDK（オプション） | 決定 | 既定はCodex。設定でComposer（`@cursor/sdk`）へ切替・相互フォールバック可。詳細は [cursor-sdk-integration.md](operations/cursor-sdk-integration.md) |
| Codex transport | stdio JSONL | 決定 | Codex App Serverの基本transportとして安定している |
| Cursor SDK bridge | Node script (`scripts/cursor-structured-turn.mts`) | 決定 | Rustから子プロセス起動。APIキーは環境変数のみ |
...
業務ツールとして使うため、画面はランディングページ風にせず、初回起動後すぐにマップ一覧またはマップ作成画面を表示する。
...
マップ操作のパフォーマンス: ノードドラッグ中はエッジラベル衝突レイアウト（`computeEdgeLabelLayout`）を開始時座標で凍結し、導線粒子アニメは共有 1 本の RAF で更新する（ドラッグ中は停止）。詳細は [flow-animation-spec.md](design/flow-animation-spec.md) を参照。
...
MVPでは、1マップにつき1 Codex threadを基本にする。
...
MVP-1ではMarkdown / CSVを必須出力とし、PDF出力は対象外にする。レポート型PDF、ダッシュボード型PDF、複数テンプレート、精密な印刷調整はBeta以降で再検討する。
...
5. 生成PDFをマップ用フォルダの`exports/`に保存する。
...
- Codexへ送る内容は、ユーザーが追加した情報ソースとマップデータに限定する。
- AIへ送信した情報ソース要約と生成結果はマップ履歴に残す。
