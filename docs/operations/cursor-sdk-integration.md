...
アプリは API キーを **DB や設定 UI に保存しません**（[phase-0-information-policy.md](../archive/phase-0-information-policy.md) 準拠）。起動時に次の `.env` を読み込み、環境変数として使います（既にシェルで設定済みの値は上書きしません）。
