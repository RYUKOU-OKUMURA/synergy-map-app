...
P0-5までの主要リスクのうち、Codex App Server stdio接続、SQLite保存、資料読み取りはmacOS PoCで検証済み。device-code flowはURL/code発行とキャンセルまで確認済みだが、実ログイン完了通知は未確認。React Flow画像化、Typst日本語PDF、sidecar同梱は未検証。
