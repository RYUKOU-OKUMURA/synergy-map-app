...
- [ ] AI視点専用出力schemaを定義する。
  - [ ] 注目点は最大3件にする。
  - [ ] カテゴリは「売上導線の欠陥」「眠っている売上資産」「利益化の盲点」にする。
  - [ ] 同じカテゴリが複数出てもよい。
  - [ ] 注目点の対象は`node`、`edge`、`map`を扱えるようにする。
- [ ] AI視点ON時だけ、売上マップ上に最大3件の注目点レイヤーを重ねる。
- [ ] AI視点ON時だけ、右側に注目点読み取りパネルを表示する。
- [ ] AI実行履歴は下部ドロワーではなく履歴画面で確認できるようにする。

完了条件:

- [ ] 通常時は売上マップが主役のまま表示され、AI視点ON時だけ注目点と読み取りパネルが重なる。
- [ ] 材料、確認すべきこと、次に試す一手、記録は下部ではなくマップ内コンテキストパネルで必要なときだけ扱える。
