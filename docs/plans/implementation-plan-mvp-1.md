...
- [x] マップ詳細を開ける。
- [x] マップを削除できる。
- [x] マップ一覧の右端メニューから、タイトル編集とマップ削除を実行できる。
- [x] マップ削除時の対象データを確認する。
  - [x] DBレコード
  - [x] 元情報ソース
  - [x] source chunks
...
## M1-20 AI視点レイヤー / コンテキストパネル再設計

- [x] AIコメントを主UIカテゴリから外す。
- [x] 下部ドロワーを主UIから廃止する。
- [x] マップ内コンテキストパネルを開閉式の作業箱として設計する。
  - [x] 材料
  - [x] 確認すべきこと
  - [x] 次に試す一手
  - [x] 記録
- [x] AI視点専用出力schemaを定義する。
  - [x] 注目点は最大3件にする。
  - [x] カテゴリは「売上導線の欠陥」「眠っている売上資産」「利益化の盲点」にする。
  - [x] 同じカテゴリが複数出てもよい。
  - [x] 注目点の対象は`node`、`edge`、`map`を扱えるようにする。
- [x] AI視点ON時だけ、売上マップ上に最大3件の注目点レイヤーを重ねる。
- [x] AI視点ON時だけ、右側に注目点読み取りパネルを表示する。
- [x] AI実行履歴は下部ドロワーではなく履歴画面で確認できるようにする。
- [x] AI視点をAI生成・保存・再表示できる。
- [x] AI視点カード単位で自由質問し、理解メモを保存できる。
- [x] AI視点カードを短い指摘、展開詳細、次のアクションに分ける。
- [x] AI視点パネルの幅をドラッグで調整し、マップ表示余白を追従させる。
- [x] 最近の理解メモを質問、AIの見立て、根拠/要点、要確認、次に試す一手に分けて表示する。
- [x] AI視点への質問中にマップのノード/導線配列を再同期しない。
- [x] AI視点カードの曖昧な「マップで見る」操作をなくし、マップ上の強調対象をカード内に明示する。
- [x] 最近の理解メモをサイドパネル常設からモーダル表示に移す。
- [x] AI視点の理解メモを1件ずつ削除できる。
- [x] Markdown / CSV出力にAI視点を含める。
- [x] AI視点ボタンと情報ソース反映ステータスを上部固定ヘッダーへ移し、右サイドパネルの閉じる操作と重ならないようにする。

完了条件:

- [x] 通常時は売上マップが主役のまま表示され、AI視点ON時だけ注目点と読み取りパネルが重なる。
- [x] 材料、確認すべきこと、次に試す一手、記録は下部ではなくマップ内コンテキストパネルで必要なときだけ扱える。
