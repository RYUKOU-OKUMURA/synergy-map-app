...
| --- | ---: | --- |
| 左ナビ | 64px | 主要ビュー切り替え |
| トップバー | 56px | マップ名、保存状態、件数、主要アクション |
| メインキャンバス | 残り全体 | 売上マップ、情報ソース、一覧ビューなど |
| マップ内コンテキストパネル | 36px閉 / 320px開 | 材料、確認すべきこと、次に試す一手、記録 |
| 右インスペクター | 300px前後 | 選択中ノード、カード、導線の編集 |
| AI視点パネル | 300px前後 | AI視点ON時だけ表示する注目点の読み取り |
...
マップ内コンテキストパネルは折りたたみを基本にする。下部ドロワーは主UIとして使わない。

- マップ内コンテキストパネル: 材料、確認すべきこと、次に試す一手、記録
- AI視点パネル: AI視点ON時だけ、最大3件の注目点を読む
- AI実行履歴: 下部ドロワーではなく履歴画面で確認する

マップ内コンテキストパネルは、左ナビとは別のマップ画面内パネルとして扱う。`材料 / 確認 / 一手 / 記録` のタブを持ち、通常時は閉じるかコンパクトに表示する。下部パネルへAIコメントや確認質問を並べる構成は採用しない。

AI視点パネルは最大3件の注目点を表示する。カテゴリは以下を基本にし、マップの状態によって同じカテゴリが複数出てもよい。

- 売上導線の欠陥: すでに存在する導線で顧客が止まる、漏れる、次に進まない場所。
- 眠っている売上資産: 存在しているのに売上導線へ十分接続されていない商品、顧客接点、発信、既存顧客、専門性。
- 利益化の盲点: 売上や反応はあるが、利益、単価、継続、高単価商品に変わっていない場所。

画面ラベルは強めにしてよいが、本文では「現在の材料では」「可能性があります」「要確認です」のように断定を避ける。

AI視点の注目点は`node`、`edge`、`map`を対象にできる。`node`と`edge`はマップ上に小さな番号マーカーを置き、対象だけを薄く強調する。`map`対象は番号マーカーを置かず、マップ全体に控えめな外枠だけを出し、AI視点パネルでは`全体`ラベル付きカードとして読む。
...
- マップ名、事業名 / 屋号 / 会社名、更新日時、情報ソース数、AI実行数、状態を表示する。
...
- 材料、確認すべきこと、次に試す一手、記録はマップ内コンテキストパネルで必要なときだけ扱う。
- AIコメントという主UIカテゴリは使わない。AIの注目点はAI視点レイヤーで拾う。
- AI視点は通常マップを置き換えず、ON時だけ注目点レイヤーと右パネルを重ねる。
