...
- [ ] Node.js / pnpm / Rust stable / Cargo が利用できることを確認する。
- [ ] Tauri v2 prerequisitesをmacOSで確認する。
- [ ] Windows側で必要なTauri prerequisitesを洗い出す。
- [ ] `codex` CLIがPATH上で実行できることを確認する。
- [ ] `typst` CLIが実行できることを確認する。
- [ ] `README.md`に開発環境の前提を追記する。

完了条件:

- [ ] `node --version`、`pnpm --version`、`rustc --version`、`cargo --version`、`codex --version`、`typst --version`の確認結果を記録している。

## P0-2 Tauri + React + Vite起動

- [ ] Tauri + React + TypeScript + Viteの初期プロジェクトを作成する。
- [ ] pnpm scriptsを整理する。
- [ ] Tauri dev serverを起動できるようにする。
- [ ] 最小のアプリシェルを表示する。
- [ ] shadcn/ui、Tailwind CSS、lucide-reactを導入する。
- [ ] TypeScript strict、ESLint、Prettierの方針を決める。

完了条件:

- [ ] `pnpm tauri dev`でデスクトップウィンドウが起動する。
- [ ] React画面に案件一覧のプレースホルダーが表示される。
