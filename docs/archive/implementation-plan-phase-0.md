...
- [x] Codex App ServerをTauri backendからstdioで安定起動できる。
- [x] ChatGPT device-code flowがWindowsユーザーに説明できるUXになる。
- [x] PDF / Excelの読み取り品質が実務最低ラインを超える。
- [x] React FlowのマップをPDFに埋め込める品質で画像化できる。
...
- Windowsユーザー向け説明文は`docs/archive/phase-0-device-code-guide.md`に記録。

完了条件:

- [ ] device-code flowでログインし、Codexのturnを開始できる。
- [x] 認証手順が非エンジニアにも説明できる。
...
- 詳細は`docs/archive/phase-0-sidecar-verification.md`に記録。
...
- MVP-1実装前の技術検証としてReact Flowのサンプルマップを追加。現在の実装はMVP-1画面へ統合済み。
- custom node `journey`とcustom edge `labeled`を追加。
- Phase 0では固定サンプル位置をコード上の`nodes`配列で保存し、再描画時に復元する。
- DOMキャプチャでPNG出力を検証。Playwright上で`PNG出力`を実行し、download成果物を確認。
...
- [x] MVP-1で必要なschemaを仮定義する。
  - [x] `ExtractedItemsOutput`
  - [x] `MapDraftOutput`
  - [x] `AiAnalysisOutput`
  - [x] `SuggestionCardsOutput`
- [x] `schema_version`を必須にする。
- [x] Codexにschema付き出力を依頼する。
- [x] Rust側でserde deserializeする。
- [x] 不正な出力を保存しない処理を作る。
- [x] `ai_runs`にrequest summaryとresponse JSONの保存パスを記録する。

検証結果:

- `src-tauri/src/ai_schema.rs`にPhase 0用の仮schemaを定義。外部JSONでは`schemaVersion`、Rust構造体では`schema_version`として扱う。
- `AiAnalysisOutput`はJSON Schemaを`turn/start`の`outputSchema`へ渡し、Codex App Serverからschema準拠JSONを受け取る。
- 直接probeで`codex app-server --listen stdio://`へschema付きturnを送り、`turn/completed`、`schemaVersion: phase0.v1`、機会2件のJSON応答を確認。
- Tauri command `run_ai_schema_poc`を追加し、schema検証済みのresponse JSONだけを`ai_runs`へ保存する。
- request側は全文promptや資料本文を保存せず、`request-summary.json`には概要、`inputHash`、schema名、schema versionだけを残す。
- `cargo test`でschema不一致を保存しないこと、`ai_runs`にrequest summary / response JSONの保存パスが残ることを確認。

完了条件:

- [x] Codex出力をschema検証してDB保存できる。
- [x] schema不一致時に再生成またはエラー表示へ回せる。

## P0-11 情報管理・ログ方針

- [x] AI送信前確認UIの最小案を作る。
- [x] MVP-1の既定送信モードを「ローカル要約だけ送る」にする。
- [x] ログに資料本文・プロンプト全文を残さない方針を実装メモに落とす。
- [x] 案件削除時に元資料、source chunks、ai_runs、exportsを削除する方針を決める。
- [x] 認証情報保存にOS keychainを使うか、Tauri Strongholdを使うか判断する。

検証結果:

- `Schema検証`に送信前確認の最小UIを追加。`要約のみで送信`を確認した場合だけschema検証を実行できる。
- `run_ai_schema_poc`は固定のPhase 0 sample summaryだけを送信し、source chunks本文は送らない。
- `request-summary.json`は概要、`inputHash`、schema名、schema versionだけを保存し、全文promptや資料本文を保存しない。
- 詳細方針は`docs/archive/phase-0-information-policy.md`に記録。
- 認証情報はSynergy Map本体では保存せず、Codex App Server / Codex CLI側の認証導線に委ねる。将来アプリ独自tokenが必要になった場合はOS keychainを第一候補にする。

完了条件:

- [x] 機密本文をログに残さない設計方針が明文化されている。
- [x] AI送信履歴に残す情報と残さない情報が決まっている。

## P0-12 Windows / macOS確認

- [x] macOSでdev buildを起動する。
- [x] macOSでファイル投入、DB保存、Codex接続、PDF出力を確認する。
- [ ] Windowsでdev buildを起動する。
- [ ] Windowsでファイル投入、DB保存、Codex接続、PDF出力を確認する。
- [x] Windows固有のパス、フォント、権限、sidecar起動問題を記録する。

検証結果:

- macOSで`pnpm tauri dev`の起動を再確認。
- macOS DB保存、サンプル材料入力、Codex stdio接続、schema付きturn、Typst PDF出力、React Flow PNG埋め込みを確認済み。
- `typst compile reports/phase-0/phase-0-report.typ reports/phase-0/phase-0-report.pdf`で2ページPDFを再生成。
- Windows実機はこの作業環境では確認できないため未実施。DB path、Codex shim、フォント、権限、sidecar起動の確認項目を`docs/archive/phase-0-platform-verification.md`に記録。
...
- macOSは完了。
- Windowsは未実機確認のため、MVP-1設計へ進む場合もWindows配布前の必須ゲートとして残す。

## Phase 0完了判定

- [ ] Go条件をすべて満たした。
- [x] 満たせなかった条件と代替案を記録した。
- [x] MVP-1に進むか、設計を見直すか判断した。
- [x] Phase 0の結果を`docs/tech-stack.md`または別レポートへ反映した。

最終判定:

- MVP-1設計・macOS PoC継続は条件付きGo。
- Windows実機確認、実ログイン完了通知、Codex App Server sidecar同梱はMVP-1配布前の必須ゲートとして残す。
