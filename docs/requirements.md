...
- MVP-1ではSynergy Map本体にAPIキーや認証情報を保存しない。Codex認証はCodex App Server / Codex CLI側の既存認証導線に委ね、Cursor SDK（Composer）の`CURSOR_API_KEY`は環境変数または`.env`から読み込む。将来、アプリ本体で独自tokenを保存する必要が出た場合は、OS標準のセキュアストレージ利用を第一候補とする。
