...
- 会議や提案の下書きに使えるMarkdown / CSVをMVP-1で出力できる。PDFはBeta以降で品質検証する。
...
Markdown / CSVを必須出力とする。PDFはTypstの日本語フォント、マップ画像埋め込み、Windows / macOS差分の追加検証が必要なため、MVP-1から外しBeta以降で扱う。ダッシュボード型PDFと複数テンプレートもMVP-1では対象外とする。
