...
ラベルは白背景、1px border、11px文字。

配置ルール:

- 基本は導線 path の中点付近（t≈0.5）に置く。
- 複数導線が近接してラベルが重なる場合は、path 上の位置と法線方向オフセットを自動調整して衝突を避ける。優先度は `bottleneck` > `strong` > `normal` > `weak` / `data_reference`。
- ラベルを path から離して配置する場合は、細い破線のリーダー線で path 上のアンカーと接続する。
- どうしても置けない場合のみ非表示とし、選択中の導線は常に表示する。
- ズーム変更時はラベル矩形をズーム補正して再配置する。
...
PDFを実装する場合は、React FlowキャンバスをDOMキャプチャ用ライブラリでPNG化して埋め込む。エクスポート用には、背景、ノード、エッジ、ラベルが読める倍率でキャプチャする。
