...
このプロジェクトは、一般的な作図ツールやマインドマップツールではなく、個人事業主・小規模会社の経営者が自分の事業情報を投入し、AIで事業構造を読み取り、売上マップ、現状分析、施策候補、確認質問、出力物までつなげる実務ツールとして設計されている。
...
一方で、このプロジェクトの勝ち筋は明確にある。汎用AIや汎用ホワイトボードは、ユーザー自身が「何を入れるか」「どう構造化するか」「どこを施策にするか」を考える必要がある。本プロジェクトは、そこを個人事業主・小規模会社の経営者向けに固定化し、事業、商品、チャネル、顧客接点、財務、未接続シナジー、確認質問まで一気通貫で扱うことに価値がある。
...
| 高 | Miro | AI図解、ホワイトボード、戦略テンプレート、共同編集 | 認知度、テンプレート、共同編集、企業利用 | 「マップの材料から事業構造と施策へ」という用途特化。Miroより迷わない入口 |
...
| SubsidyDraft AI / 補助金AI | 事業概要から申請書下書きを作る | 初回ハードルが低く、出力物が明確 | 申請書ではなく、継続的な経営整理、相談準備、振り返りに使う |
...
月額980円という価格は、Miro、Lucid、Whimsical、Napkin、MyMap、Xmindなどの低価格AI/図解ツールと比較されやすい。ただしユーザー側には、ChatGPT課金や関連ランニングコストも乗る。したがって「安いから使う」ではなく「自分の事業整理で毎月使う理由」が必要になる。

価格に対する価値の出し方:

- 初回: マップの材料から短時間で事業構造が見える。
...
AI図解ツールは多いが、施策候補、期待インパクト、実装コスト、確認指標、次に確認すべき質問まで出すものは少ない。ここをプロダクトの継続利用理由にする。
...
- バラバラの事業情報を、AIが売上マップと施策候補に整理。
- 事業の材料整理を、事業構造マップと確認質問に変える。
